...
var PagesIndicator = function PagesIndicator(_ref) {
	...
	return React.createElement('div', { className: pagesClass, style: css ? {} : styles.pages }, page, '/', pages);
};
	...
	function PDFViewer() {
		...
		return (
			(_ret = ((_temp = ((_this = possibleConstructorReturn(
				this,
				(_ref = PDFViewer.__proto__ || Object.getPrototypeOf(PDFViewer)).call.apply(_ref, [ this ].concat(args))
			)),
			_this)),
			(_this.state = {
				pages: 0,
			...
			value: function render() {
				...
				if (!hideNavbar && pages > 0) {
					nav =
						!navigation ||
						(navigation &&
							(typeof navigation === 'undefined' ? 'undefined' : _typeof(navigation)) === 'object')
				...
				return React.createElement(
					...
					React.createElement('div', { onClick: onDocumentClick }, pdf),
					nav
				);
			}
		}
